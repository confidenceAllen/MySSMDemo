...
layui.config({
	...
	function lockPage(){
		layer.open({
			...
			content : '	<div class="admin-header-lock" id="lock-box">'+
							'<div class="admin-header-lock-img"><img src="images/userface4.jpg"/></div>'+
							'<div class="admin-header-lock-name" id="lockUserName">call me by</div>'+
							'<div class="input_btn">'+
								'<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />'+
								'<button class="layui-btn" id="unlock">解锁</button>'+
							'</div>'+
							'<p style="color: red">password 123</p>'+
						'</div>',
			closeBtn : 0,
			shade : 0.9
		})
	...
	function showNotice(){
		layer.open({
			...
			'系统使用layui作为前台展示页面、爬虫作为数据支撑、mybatis-plus作为持久化数据框架、后续增加shrio作为权限控制。完善这个初始化demo</p></div>',
...
function donation(){
	layer.tab({
		...
		tab : [{
			title : "弹窗1",
			content : "<div style='padding:30px;overflow:hidden;background:#d2d0d0;'><img src='images/userface5.jpg'></div>"
		},{
			title : "弹窗2",
			content : "<div style='padding:30px;overflow:hidden;background:#d2d0d0;'><img src='images/userface4.jpg'></div>"
		}]
	})
}
